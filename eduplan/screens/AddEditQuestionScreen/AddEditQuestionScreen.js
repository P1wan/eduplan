  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (isEditing) {
        ...
        if (questionIndex > -1) {
          allQuestions[questionIndex] = { ...allQuestions[questionIndex], enunciado, alternativas: [alternativa1, alternativa2, alternativa3, alternativa4], respostaCorreta, disciplina, ano };
        }
      } else {
        const newQuestion = { id: `local_${new Date().getTime()}`, enunciado, alternativas: [alternativa1, alternativa2, alternativa3, alternativa4], respostaCorreta, disciplina, ano, isCanonical: false };
        if (isAdapting) {
          newQuestion.type = 'ADAPTATION';
          newQuestion.originalRef = { collection: 'canonical_questions', docId: originalQuestion.id };
        } else {
      ...
      // --- AQUI ESTÁ A MUDANÇA ---
      // Navega de volta para a lista e passa um parâmetro para forçar a atualização
    ...
    <SafeAreaView style={styles.container}>
      <ScrollView>
        <View style={styles.formContainer}>
          ...
          <TextInput style={styles.input} value={respostaCorreta} onChangeText={setRespostaCorreta} />
